...
- Reduced spacing in module and announcement display
  - Removed excess blank lines for cleaner output
  - Tightened vertical spacing between title and content
  - Display now shows blue links inline with content

### Changed

- **Enhanced Course Selection in pickCourse**: Integrated course name search directly
  - Support for course name arguments (e.g., `canvas mod cloud`) without separate getCanvasCourse
  - Auto-selection when single match found
  - Interactive list when multiple matches found

- **Display Without Tables**: Content display now uses clean format without boxes
  - Module pages display without borders
  - Announcement details display without borders
  - Selection screens still use tables for better organization
