...
describe('Submit Command - File Summary Table', () => {
  test('file summary table displays selected files', () => {
    ...
    const table = new Table(columns, { showRowNumbers: true, title: 'Selected Files' });
    table.addRow({ filename: 'assignment1.pdf', size: '2.5 MB' });
    table.addRow({ filename: 'screenshot.png', size: '1.2 MB' });
    table.addRow({ filename: 'code_submission_with_a_very_long_filename.zip', size: '15.8 MB' });
  ...
  test('submission summary table shows assignment details', () => {
    ...
    const table = new Table(columns, { showRowNumbers: false, title: 'Submission Summary' });
    table.addRow({ field: 'Assignment', value: 'Project 1: Web Development' });
    table.addRow({ field: 'Course', value: 'CS 101 - Introduction to Programming' });
    table.addRow({ field: 'Due Date', value: '2025-12-20 11:59 PM' });
    table.addRow({ field: 'Files', value: '3 file(s)' });
    table.addRow({ field: 'Mode', value: 'DRY RUN - No files will be uploaded' });
  ...
  test('file browser table with extensions filter hint', () => {
    ...
    const table = new Table(columns, { showRowNumbers: true, title: 'Select Files (Allowed: .pdf, .docx, .zip)' });
  ...
  test('submission result table shows upload status', () => {
    ...
    const table = new Table(columns, { showRowNumbers: false, title: '✓ Submission Complete' });
...
describe('Submit Command - Edge Cases', () => {
  ...
  test('handles very long filenames with truncation', () => {
    ...
    const table = new Table(columns, { showRowNumbers: false });
    table.addRow({
      filename: 'this_is_an_extremely_long_filename_that_definitely_exceeds_the_column_width_and_needs_truncation.pdf',
      size: '5.0 MB'
    });
