...
describe('Display Library - Table Rendering', () => {
  test('table truncates cell to given width', () => {
    ...
    const longName = 'Assignment 1: Very long unnamed assignment that should be cut';
    ...
    const dataLine = rendered.split('\n').find(l => l.includes('│') && !l.includes('Name') && /[A-Za-z0-9]/.test(l));
    expect(dataLine).toBeDefined();

    const visible = stripAnsi(dataLine!);
    const content = visible.slice(visible.indexOf('│') + 1, visible.lastIndexOf('│')).trim();
  ...
  test('table with multiple columns adapts width', () => {
    ...
    table.addRow({ id: '2', name: 'Assignment Two with a very long name', status: 'Pending' });
  ...
  test('table with row numbers displays correctly', () => {
    ...
    const table = new Table(columns, { showRowNumbers: true, rowNumberHeader: '#' });
  ...
  test('table with title displays title', () => {
    ...
    const table = new Table(columns, { title: 'Test Table', showRowNumbers: false });
