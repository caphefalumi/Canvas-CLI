...
export async function showProfile(options: ProfileOptions = {}): Promise<void> {
  try {
    ...
    const verboseFields: [string, string][] = [
      ...
      ['Created', (user as any).created_at ? new Date((user as any).created_at).toLocaleString() : 'N/A'],
    ];

    const fields = options.verbose ? [...basicFields, ...verboseFields] : basicFields;
    ...
    const colLabel = Math.max(maxLabelLen + 1, Math.min(20, Math.floor(available * 0.30)));
    ...
    // Header
    console.log(
      chalk.gray('│ ') + chalk.cyan.bold(pad('Field', colLabel)) + chalk.gray('│ ') +
      chalk.cyan.bold(pad('Value', colValue)) + chalk.gray('│')
    );
    ...
    fields.forEach(([label, value]) => {
      ...
      console.log(
        chalk.gray('│ ') + chalk.white(pad(label, colLabel)) + chalk.gray('│ ') +
        chalk.white(pad(displayValue, colValue)) + chalk.gray('│')
      );
    });
