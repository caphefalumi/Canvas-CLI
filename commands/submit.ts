...
export async function submitAssignment(
  ...
  try {
    ...
    if (courseName) {
      ...
      if (!options.all) {
        ...
        if (displayCourses.length === 0) {
          console.log(
            chalk.yellow(
              "No starred courses found. Showing all courses instead.",
            ),
          );
      ...
      const courseChoice = await askQuestion(
        rl,
    ...
    const uploadableAssignments = assignments.filter(
      (a) => a.submission_types && a.submission_types.includes("online_upload"),
    );

    if (uploadableAssignments.length === 0) {
      console.log(
        chalk.red("Error: No assignments accept file uploads in this course."),
      );
      rl.close();
      return;
    }
    ...
    while (true) {
      ...
      if (
        !assignmentChoice.trim() ||
        assignmentChoice === ".." ||
        assignmentChoice.toLowerCase() === "back"
      ) {
        console.log(chalk.yellow("Submission cancelled."));
        rl.close();
        return;
      ...
      if (
        assignmentIndex < 0 ||
        assignmentIndex >= uploadableAssignments.length ||
        isNaN(assignmentIndex)
      ) {
        console.log(
          chalk.red("Invalid assignment selection. Please try again."),
        );
        continue;
      }
      ...
      clearLines(1);
      console.log(
        chalk.green(`✓ Selected assignment: ${selectedAssignment.name}`),
      );
      break;
    }
    ...
    if (options.file) {
      ...
      console.log(
        chalk.green(`✓ Using specified file: ${path.basename(filePath)}`),
      );
    } else {
      ...
      try {
        ...
        const allowed =
          (selectedAssignment &&
            (selectedAssignment as any).allowed_extensions) ||
          undefined;
    ...
    for (let i = 0; i < filesToSubmit.length; i++) {
      ...
      try {
        console.log(
          chalk.white(
            `Uploading ${i + 1}/${filesToSubmit.length}: ${path.basename(file)}...`,
          ),
        );
        if (!options.dryRun) {
    ...
    try {
      ...
      const config = await import("../lib/config.js").then((c) =>
        c.loadConfig(),
      );
