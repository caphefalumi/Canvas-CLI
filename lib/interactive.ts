...
export function getSubfoldersRecursive(
 ...
 */
export function getFilesMatchingWildcard(
  pattern: string,
 ...
 */
export async function selectFilesImproved(
  rl: readline.Interface,
  currentDir: string = process.cwd(),
): Promise<string[]> {
  ...
  while (true) {
    if (selectedFiles.length > 0) {
      ...
      console.log(
        chalk.cyan.bold(`Currently selected (${selectedFiles.length} files):`),
      );
    ...
    if (input.toLowerCase() === "remove") {
      ...
      const removeChoice = await askQuestion(
        rl,
        chalk.bold.cyan(
    ...
    try {
      ...
      if (stats.isDirectory()) {
        ...
        console.log(chalk.cyan("-".repeat(50)));
        console.log(
          chalk.cyan(`Total size: ${(totalSize / 1024).toFixed(1)} KB`),
        );
        const confirmFolder = await askConfirmation(
          rl,
          chalk.bold.cyan(
            `Add all ${collectedFiles.length} files from this folder?`,
          ),
          true,
        );
        if (confirmFolder) {
          const newFiles = collectedFiles.filter(
            (f) => !selectedFiles.includes(f),
          );
          selectedFiles.push(...newFiles);
          console.log(
            chalk.green(
              `Added ${newFiles.length} new files (${collectedFiles.length - newFiles.length} already selected)`,
            ),
          );
        }
        continue;
      ...
      if (selectedFiles.includes(filePath)) {
        console.log(
          chalk.yellow(`File already selected: ${path.basename(filePath)}`),
        );
        continue;
      }
