...
async function ensureConfig(): Promise<boolean> {
  if (!configExists()) {
    ...
    const setup = await askQuestion(
      rl,
      "Would you like to set up your configuration now? (Y/n): ",
    );
    rl.close();
    ...
    if (setup.toLowerCase() === "n" || setup.toLowerCase() === "no") {
      ...
      console.log(
        "Or set environment variables: CANVAS_DOMAIN and CANVAS_API_TOKEN",
      );
      process.exit(1);
    }
  ...
  if (!config || !config.domain || !config.token) {
    console.log(
      'Invalid configuration found. Please run "canvas config setup" to reconfigure.',
    );
    process.exit(1);
  }
